...
export function calculateUserRecord(
  ...
  const currency = (userEntry.exch as any).currency ?? "PAISA";

  // If exchange currency is RUPEE, points are in rupees → multiply by 100 to normalize to paisa
  const normalizedGross = currency === "RUPEE" ? gross * 100 : gross;
  ...
  if (!hasStar) {
    point = round2(normalizedGross);
    amtGross = round2((normalizedGross * RATE) / 10000);
  } else {
    amtGross = round2(normalizedGross / 100);
    point = round2(normalizedGross / 100) / RATE * 10000;
  }
  ...
  const adrcr = normalizedGross < 0 ? "D" : "C";
